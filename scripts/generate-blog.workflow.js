...
const DRAFT_SCHEMA = {
  type: 'object',
  properties: {
    body_md: { type: 'string', description: '마크다운 본문 (1500~3000자, # ## ### 헤딩 + 표 + 리스트 + > 인용 + [링크])' },
...
function writePrompt(t, research, revisionNote) {
  ...
  return `당신은 돌봄지기의 라이터입니다. P1 큐레이터 — 1차 자료를 친절히 정리하되 자격 주장·권유는 하지 않습니다.
...
리서치 데이터(이것을 본문에 최소 3건 인용):
${JSON.stringify(research.facts, null, 2)}
핵심 개념: ${(research.key_concepts || []).join(', ')}

**작성 규칙:**
- 마크다운만 사용. 지원 문법: # (대제목 h2), ## (중제목 h3), ### (소제목), 표(| |), 리스트(- 또는 1.), 인용(>), **굵게**, [링크](url)
- 분량 1500~3000자, H2(=# ) 헤딩 4~7개
- 1차 데이터 3건 이상 인용하며 출처 명시 (예: "통계청에 따르면...")
- 내부링크 2~4개를 본문에 자연스럽게: [돌봄로봇 제품](/robot), [지원사업 안내](/support), [이용 가이드](/guide), [제품 비교](/compare)
- 첫 문단(훅): 매크로에 맞게 (질문/통계/장면/대비 중 하나). "안녕하세요" 류 클리셰 절대 금지
- 결말: 요약 또는 다음 행동 안내. "이상으로 마치겠습니다" 류 금지
- summary: AnswerBlock용 250자 이내 핵심 답변 (별도)
- 표나 리스트를 1개 이상 포함 (가독성)
...
**평가 항목 (8차원 100점):**
- 독창성 20 / 1차데이터 인용 20 / 구조·가독성 15 / 페르소나 일관성(P1 큐레이터) 10 / AEO(질문 즉답성) 10 / 내부링크·본문리듬 10 / 문장완성도 10 / 검색의도 충족 5
